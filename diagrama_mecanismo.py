...
def geom(THETA2):
    ...
    # --- 4B#3 (DIP, corregido) -> distal ---
    thfp_=P['thfp']
    alpha1=thfp_+np.deg2rad(BETA1)-thfm
    Acrk=IFP+Lpc*np.array([np.cos(thfp_+np.deg2rad(BETA1)),np.sin(thfp_+np.deg2rad(BETA1))])
    ...
    Brok=IFD+Lpd*np.array([np.cos(thfd+np.deg2rad(BETA2)),np.sin(thfd+np.deg2rad(BETA2))])
...
checks=[
 ...
 ("|CRK3-IFP|=Lpc",dist('CRK3','IFP'),Lpc),
 ("|ROK3-IFD|=Lpd",dist('ROK3','IFD'),Lpd),
 ("|ROK3-CRK3|=Lac",dist('ROK3','CRK3'),Lac),
]
...
def bar(ax, n1, n2, color, lw=2.5, label=None, ls='-', z=3):
    ...
    if label:
        ...
        ax.annotate(label, m, fontsize=8, color=color, ha='center', va='center',
                    bbox=dict(boxstyle='round,pad=0.12', fc='white', ec=color, alpha=0.85),
                    zorder=z+1)

def joint(ax, name, txt=None, dx=2.2, dy=2.2, c='k', ms=7):
    p = pt(name)
...
# --- Bancada / marco fijo ---
bar(ax,'A','B',C_FRAME,lw=3,label='Bancada1 (2*r3=%.0f)'%(Bancada1),z=2)
bar(ax,'A','MCF',C_FRAME,lw=3,label='Bancada2 (d=%.0f)'%(Bancada2),z=2)
# soporte gris de la bancada
ax.fill([pt('A')[0],pt('B')[0],pt('MCF')[0]],
        [pt('A')[1],pt('B')[1],pt('MCF')[1]],color=C_FRAME,alpha=0.06,zorder=1)

# --- Engranes (motor unico) en A (manivela 4B/5B th1) y B (manivela 5B th2) ---
...
# --- Primer mecanismo de 4 barras (manivela Link4 compartida) ---
bar(ax,'M4','S1',C_4B1,label='Link5',z=4)
bar(ax,'MCF','S1',C_4B1,label='c=v(hsp^2+dsp^2)',z=4)

# --- Segundo mecanismo de 5 barras ---
bar(ax,'S1','S2',C_5B2,label='fp-2dsp',z=4)
bar(ax,'S2','P2',C_5B2,label='Link7',z=4)
bar(ax,'P','P2',C_5B2,label='Link6',z=4)
# (Link3 M4-P y crank Link1 ya dibujados; sirven de entrada al 5B#2)

# --- Segundo mecanismo de 4 barras ---
bar(ax,'P2','P3',C_4B2,label='Link8',z=4)
bar(ax,'IFP','P3',C_4B2,label='c2=%.2f'%c2,z=4)
bar(ax,'IFP','S2',C_4B2,ls='--',lw=1.6,label='d2 (bancada)',z=3)

# --- Tercer mecanismo de 4 barras (DIP, NUEVO) ---
bar(ax,'IFP','CRK3',C_4B3,label='Lpc=%.0f (BETA1=%.0f)'%(Lpc,BETA1),z=5)
bar(ax,'CRK3','ROK3',C_4B3,label='Lac=%.2f'%Lac,z=5)
bar(ax,'IFD','ROK3',C_4B3,label='Lpd=%.0f (BETA2=%.0f)'%(Lpd,BETA2),z=5)

# --- Falanges (cuerpos rigidos) ---
bar(ax,'MCF','IFP',C_PHAL,lw=6,label='Falange proximal (fp=%.0f)'%fp,z=2)
bar(ax,'IFP','IFD',C_PHAL,lw=6,label='Falange medial (fm=%.0f)'%fm,z=2)
bar(ax,'IFD','TIP',C_PHAL,lw=6,label='Falange distal (fd=%.0f)'%fd,z=2)

# --- Articulaciones etiquetadas ---
joint(ax,'A','A (-r3,0)',c=C_FRAME)
joint(ax,'B','B (r3,0)',c=C_FRAME)
joint(ax,'MCF','MCF',c=C_PHAL,dx=-12,dy=-5)
joint(ax,'M4','M4',c=C_5B1)
joint(ax,'P','P',c=C_5B1)
joint(ax,'J2','J2',c=C_5B1)
joint(ax,'S1','S1',c=C_4B1,dx=-9,dy=2)
joint(ax,'S2','S2',c=C_5B2)
joint(ax,'P2','P2',c=C_5B2)
joint(ax,'P3','P3',c=C_4B2)
joint(ax,'IFP','IFP (PIP)',c=C_PHAL,dx=2,dy=-5)
joint(ax,'IFD','IFD (DIP)',c=C_PHAL)
joint(ax,'CRK3','',c=C_4B3,ms=5)
joint(ax,'ROK3','',c=C_4B3,ms=5)
joint(ax,'TIP','Punta',c=C_PHAL)

# --- Leyenda de subsistemas ---
...
leg=[Line2D([0],[0],color=C_FRAME,lw=3,label='Bancada / marco fijo'),
     ...
     Line2D([0],[0],color=C_4B1,lw=2.5,label='Mecanismo 4 barras #1 (falange proximal)'),
     Line2D([0],[0],color=C_5B2,lw=2.5,label='Mecanismo 5 barras #2'),
     Line2D([0],[0],color=C_4B2,lw=2.5,label='Mecanismo 4 barras #2 (falange medial)'),
     Line2D([0],[0],color=C_4B3,lw=2.5,label='Mecanismo 4 barras #3 NUEVO (falange distal/DIP)'),
     Line2D([0],[0],color=C_PHAL,lw=6,label='Falanges (cuerpos rigidos del dedo)')]
...
plt.tight_layout()
plt.savefig('diagrama_mecanismo_completo.png',dpi=150)
plt.close()
print("\nGuardado diagrama_mecanismo_completo.png")
for k in ['A','B','MCF','M4','J2','P','S1','S2','P2','P3','IFP','IFD','TIP']:
